/**
 * pack-dist.mjs — wiki-core 5 패키지를 .tgz 로 pack (publish-like).
 *
 * `pnpm pack` 이 `workspace:*` 를 publish 시점 버전(`^0.1.0` / `0.1.0`)으로 자동 변환.
 * → npm 호환 .tgz 5종이 dist-tarballs/ 에 출력.
 *
...
const PACKAGES = ['core', 'storage', 'router', 'renderer', 'extractor'];
...
console.log(`[pack-dist] done — 5 tarballs in ${dest}`);
