...
>
> **세션 시작 시**: GitHub pull → `git log --oneline -10` → 이 파일 직접 Read → `docs/` 3 명세서 정독 → 작업 시작
> **세션 종료 시**: 진행 상황을 이 파일 하단 "마지막 세션 정보"에 박제 → push
...
4. **에드워드를 검증자로 활용** — 추상화 결정 시 3 도메인 owner의 의도를 직접 묻기보다 에드워드를 통해 확인 (도메인 owner들과 직접 대화 X).
